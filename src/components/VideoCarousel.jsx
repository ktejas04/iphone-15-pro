...
const VideoCarousel = () => {
    ...
    useEffect(() => {
        ...
        let span = videoSpanRef.current; //span for current video
        ...
        if (span[videoId]) {
            ...
            let anim = gsap.to(span[videoId],{
                onUpdate: () => {
                    ...
                    const progress = Math.ceil(anim.progress() * 100); //in percentage
    ...
    useEffect(() => {
        if (loadedData.length > 3){
             ...
             //if we have reached the last video and it is not playing then pause it
             else
     ...
     const handleProcess = (type,i) => { //type of video
        switch(type){
            case "video-end" :
  ...
  return (
    <>
       ...
       <div className="relative flex-center mt-10">
            ...
            {videoRef.current.map((_,i) => ( //_is video - not needed
                    <span
                    key={i}
                    ref={(el) => (videoDivRef.current[i] = el)} //new video added to vdr array
                    className="mx-2 w-3 h-3 bg-gray-200 rounded-full relative cursor-pointer"  
                    >
                        <span className="absolute h-full w-full rounded-full" ref={(el) => (videoSpanRef.current[i] = el)}/> {/*new span added to vsr array*/} 
                    </span>
                ))}
            </div>
